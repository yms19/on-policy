...
class MultiAgentEnv(gym.Env):
    ...
    def __init__(self, world, reset_callback=None, reward_callback=None,
        ...
        for agent in self.agents:
            ...
            if self.discrete_action_space:
                u_action_space = spaces.Discrete(world.dim_p * 2 + 3)
            else:
    ...
    def step(self, action_n):
        ...
        return obs_n, reward_n, done_n, info_n, avail_n
    ...
    def reset(self):
        ...
        for agent in self.agents:
            obs_n.append(self._get_obs(agent))
            avail = np.ones(self.action_space[0].n)
            avail[-1] = 0
        ...
        for i in range(len(self.viewers)):
            ...
            for e, entity in enumerate(self.world.entities):
                ...
                if 'agent' in entity.name:
                    ...
                    if not entity.dummy:
                        if entity.detected:
                            self.comm_geoms[e][0].set_color(1,0,0)
                        else:
                            self.comm_geoms[e][0].set_color(0,0,0)
                    
                    if entity.adversary:
                        track = rendering.make_circle(0.005)
                        track_xform = rendering.Transform()
                        track.set_color(0, 0, 0)
                        track_xform.set_translation(*entity.state.p_pos)
                        track.add_attr(track_xform)
                        self.viewers[0].add_geom(track)
            ...
            # self.comm_geoms_xform[0].set_scale((1 + 0.005*self.current_step), 
            #                                     (1 + 0.005*self.current_step))
